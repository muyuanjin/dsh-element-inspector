window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function stableClasses(value) {
      return String(value || '').split(/\s+/).filter(item => item.length >= 3 && !/^(?:css-|sc-|jsx-|ng-|ant-|el-)/i.test(item) && !/^[a-f0-9]{8,}$/i.test(item)).slice(0, 5)
    }
    function hiddenRule(info) {
      const attrs = {}
      for (const key of ['data-plugin', 'data-testid', 'data-test-id', 'aria-label', 'role']) if (info.attrs?.[key]) attrs[key] = String(info.attrs[key]).slice(0, 120)
      return { id: String(info.id || '').slice(0, 120), classes: stableClasses(info.classes), attrs, text: normalizeText(info.text), tag: String(info.tag || '').toLowerCase() }
    }
    function ruleMatches(element, rule) {
      if (!(element instanceof Element)) return false
      if (rule.id) return element.id === rule.id
      if (rule.tag && element.tagName.toLowerCase() !== rule.tag) return false
      const attrs = rule.attrs || {}
      const attrKeys = Object.keys(attrs)
      if (attrKeys.length && !attrKeys.every(key => element.getAttribute(key) === attrs[key])) return false
      const classes = rule.classes || []
      if (classes.length && !classes.every(name => element.classList.contains(name))) return false
      if (rule.text && normalizeText(element.innerText || element.textContent) !== rule.text) return false
      return attrKeys.length > 0 || classes.length > 0
    }
    function targetInfo(target) {
      ...
      return { text: normalizeText(target.innerText || target.textContent), aria: target.getAttribute('aria-label') || '', id: target.id || '', classes: typeof target.className === 'string' ? target.className.slice(0, 240) : '', role: target.getAttribute('role') || '', tag: target.tagName || '', attrs, ancestors, owner }
    }
    ...
    function apply(ctx) {
      ...
      const close = () => { active = false; current = undefined; selectedInfo = undefined; previousView = ''; clearTimeout(noticeTimer); root?.remove(); root = undefined; mask = undefined }
      ...
      const applyHidden = () => {
        ...
        for (const element of document.body.querySelectorAll('*')) {
          ...
          const hidden = prefs.hidden.some(rule => ruleMatches(element, rule))
          if (hidden) {
      ...
      const scheduleHidden = () => { clearTimeout(hideTimer); hideTimer = setTimeout(applyHidden, 60) }
      const settings = () => {
        active = false
      ...
      const pick = async event => {
        ...
        try {
          ...
          const canHide = Boolean(hiddenRule(info).id || hiddenRule(info).classes.length || Object.keys(hiddenRule(info).attrs).length)
          ...
          const actions = `<div class="dei-actions">${canHide ? '<button type="button" class="dei-button dei-primary dei-hide-current">永久隐藏此元素</button>' : ''}<button type="button" class="dei-button dei-outline dei-settings">插件设置</button></div>`
          render(panel(heading, `${summary}${text}${actions}${hits ? `<div class="dei-section-title">${data.certainty === 'confirmed' ? '判断依据' : '候选插件'}</div>${hits}` : ''}`))
        } catch (error) { render(panel('分析失败', `<p class="dei-error">${esc(error)}</p><div class="dei-actions"><button type="button" class="dei-button dei-primary dei-settings">打开设置</button></div>`)) }
      }
      const action = async event => {
        ...
        if (element?.closest('.dei-hide-current') && selectedInfo) { const rule = hiddenRule(selectedInfo); const hidden = prefs.hidden.some(item => JSON.stringify(item) === JSON.stringify(rule)) ? prefs.hidden : [...prefs.hidden, rule]; if (await savePreference('hidden', hidden)) { applyHidden(); settings() }; return }
        ...
        try {
          ...
          if (!response.ok) throw new Error((await response.json()).message || `请求失败 (${response.status})`)
      ...
      ctx.effect(() => () => { clearTimeout(hideTimer); unsubscribePreferences(); observer.disconnect(); document.removeEventListener('keydown', key, true); document.removeEventListener('mousemove', move, true); document.removeEventListener('click', action, true); document.removeEventListener('click', click, true); close(); document.getElementById(STYLE_ID)?.remove() }, 'dsh-element-inspector: listeners')
    }
